...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="aliyutimileyin2340@gmail.com" password="A123"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
            ...
            self.assertIn('aliyutimileyin2340@gmail.com', result)
            self.assertIn('A123', result)
            cursor.close()
            dbc.close()
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="aliyutimileyin2340@gmail.com", password="A123")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('aliyutimileyin2340@gmail.com', result)
            self.assertIn('A123', result)
            self.assertIn('aliyutimileyin2340@gmail.com', cout.getvalue())
            self.assertIn('A123', cout.getvalue())
            cursor.close()
            dbc.close()
